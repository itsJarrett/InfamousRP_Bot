...
namespace InfamousRP_Bot
{
    ...
    internal class Program
    {
        ...
        {
            if (!Settings.getSettings().ProductionGuildServerNames.Contains(beforeGuildUser.Guild.Name) || 
                !Settings.getSettings().DevelopmentGuildServerNames.Contains(beforeGuildUser.Guild.Name) &&
                beforeGuildUser.IsBot || beforeGuildUser.IsWebhook) return Task.CompletedTask;
        ...
        private Task ClientOnMessageReceived(SocketMessage socketMessage)
        {
            ...
            if (waitingProduction.Contains(userId) || waitingDevelopment.Contains(userId))
            {
                ...
                {
                    Console.WriteLine("UPDATE WHITELIST STATUS PRODUCTION");
